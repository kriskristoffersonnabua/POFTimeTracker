(function($) {
    ...
    var logActivity = {
        ...
        init: function($el) {
            ...
            self.gteChartData(
                $chartEl,
                this.options.date.startDate.format("YYYY-MM-DD"),
                this.options.date.endDate.format("YYYY-MM-DD")
            );
        }
    };
        ...
        init: function($el) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $("#registration_usage_from").text(
                        (100 / count) * parseInt(response.registration_form)
                    );
                    $("#registration_usage_google").text(
                        (100 / count) * parseInt(response.google)
                    );
                    $("#registration_usage_facebook").text(
                        (100 / count) * parseInt(response.facebook)
                    );
                    $("#registration_usage_twitter").text(
                        (100 / count) * parseInt(response.twitter)
                    );
